...
while (trialCount < trialTotal)
    ...
    R1_Act = ( (r2_Act)^2+(r3_Act)^2+(r4_Act)^2+(4*d)^2 ) / ...
        ( -2*(r2_Act+r3_Act+r4_Act) );
    
    R2_Act = R1_Act + r2_Act;
    R3_Act = R1_Act + r3_Act;
    R4_Act = R1_Act + r4_Act;
    
    xS_Act = (-R1_Act^2 + R2_Act^2 + R3_Act^2 - R4_Act^2 + 2*d^2) / (4*d);
    yS_Act = ( R1_Act^2 - R2_Act^2 + R3_Act^2 - R4_Act^2 + 2*d^2) / (4*d);
    zS_Act = ( R1_Act^2 + R2_Act^2 - R3_Act^2 - R4_Act^2 + 2*d^2) / (4*d);
    ...
    if ( isreal(xS_Est) && isreal(yS_Est) && isreal(zS_Est) && ...
            isreal(xS_Act) && isreal(yS_Act) && isreal(zS_Act) )
        
        trialCount = trialCount + 1;
        
        % Converting azimuths from radians to degrees and wrapping them so
        % that they fall between 0 and 360 degrees
        azimuthH_Act = wrapTo2Pi(atan2(yS_Act,xS_Act)) * (180/pi);
        azimuthV_Act = wrapTo2Pi(atan2(zS_Act,xS_Act)) * (180/pi);
