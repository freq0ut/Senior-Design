...
for trialCount = 1:trialTotal;
    ...
    DATA_RAW(1,:) = DC_Offset(1) + (1.2+0.2*rand())*cos(2*pi*fPing*(t+tD_Act(1)));
    DATA_RAW(2,:) = DC_Offset(2) + (1.2+0.2*rand())*cos(2*pi*fPing*(t+tD_Act(2)));
    DATA_RAW(3,:) = DC_Offset(3) + (1.2+0.2*rand())*cos(2*pi*fPing*(t+tD_Act(3)));
    DATA_RAW(4,:) = DC_Offset(4) + (1.2+0.2*rand())*cos(2*pi*fPing*(t+tD_Act(4)));
    ...
    if (fig1_On == true)
        ...
        figure(1)
            subplot(2,2,1);
                ...
                string111 = sprintf('f_{samp} = %0.3f [kHz]', fADC/1e3);
